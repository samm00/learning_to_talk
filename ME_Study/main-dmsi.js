...
Sequence('pid_check', 'caregiver_instructions', 'init', 'wait_for_child', 'child_welcome', 'instructions', 'practice1', 'practice', 'progress1', 'quest1', 'images', 'upload', 'finish');
//Sequence( 'init', 'images', 'upload', 'finish');

InitiateRecorder('https://l2t-umd-dmsp.xyz/info.php').label('init');

// ID: ABCD
//    A = 0 (AAE), 1 (GAE)
//    B = 0-3 (Which csv #) [Necessary?]
//    CD = ID #
...
newTrial('child_welcome',
    defaultText.center().print(),
    newText('Welcome!'),
    newText('Press the space bar again to start the game'),
...
newTrial('instructions', 
    ...
    newImage('intro', img_path +  'IntroExperimentDisplayScreen_b.jpg').center().print(),
...
Template('mytable' , row =>
    PennController('practice',
        ...
        newImage('topleft', img_path + row.tl).size(300, 300).log().print(),
        newImage('topright', img_path + row.tr).size(300, 300).log().print(),
        newImage('bottomleft', img_path + row.bl).size(300, 300).log().print(),
        newImage('bottomright', img_path + row.br).size(300, 300).log().print(),
...
newTrial('quest1',
    ...
    newImage('questimg', img_path + 'Reinforcer_QuestMap1b.jpg').center().print(),
...
Template(csv, row =>
    PennController('images',
        ...
        getVar('trial_num').value % 6 === 0 ? newImage('reinforcer', img_path + 'Reinforcer_' + (getVar('trial_num').value / 6) + 'b_pp.jpg').center().print() : NaN,
        getVar('trial_num').value % 6 === 0 ? newAudio('Reinforcer_GreatJob', aud_path + 'Reinforcer_GreatJob.mp3').play().wait().log() : NaN,
        getVar('trial_num').value % 6 === 0 ? getImage('reinforcer').remove() : NaN,

        getVar('trial_num').set(parseInt(trial_num++)),

        getVar('trial_num').value % 6 === 0 ? newImage('questimg', img_path + 'Reinforcer_QuestMap' + ((getVar('trial_num').value / 6) + 1) + 'b_pp.jpg').center().print() : NaN,
        getVar('trial_num').value % 6 === 0 ? newAudio('QuestMap_Mission' + ((getVar('trial_num').value / 6) + 1) + '.mp3', aud_path + 'QuestMap_Mission' + ((getVar('trial_num').value / 6) + 1) + '.mp3').play().wait().log() : NaN,
        getVar('trial_num').value % 6 === 0 ? newFunction('space_aud', () => {
            var audio = new Audio(aud_path + 'Press_Spacebar.mp3');
            console.log(audio);
            audio.play();
        }).call() : NaN,
        getVar('trial_num').value % 6 === 0 ? newTimer('wait' + counter++, 800).start().wait() : NaN,
        getVar('trial_num').value % 6 === 0 ? newButton('space', 'spacebar').center().print() : NaN,
        getVar('trial_num').value % 6 === 0 ? newTimer('wait' + counter++, 600).start().wait() : NaN,
        getVar('trial_num').value % 6 === 0 ? newKey(' ').wait() : NaN,
        getVar('trial_num').value % 6 === 0 ? getImage('questimg').remove() : NaN,
        getVar('trial_num').value % 6 === 0 ? getButton('space').remove() : NaN,
        ...
        newFunction('aud', () => {
            ...
            if(row.Speaker == 'GAE' && speaker[row.Condition + '_name'] == 'Holly') {
                audio_file += speaker[row.Condition + '_name'] + '_' + row.Speaker + '_W_DMSI.mp3';
            } else if (row.Speaker == 'GAE') {
                audio_file += speaker[row.Condition + '_name'] + '_' + row.Speaker + '_B_DMSI.mp3';
            } else {
                audio_file += speaker[row.Condition + '_name'] + '_' + row.Speaker + '_DMSI.mp3';
            }
            audio = new Audio(aud_path + audio_file);
            console.log(audio);
            audio.play();
        }).call(),

        newTimer('wait', 8000).start().wait(),
        ...
        newFunction('aud2', () => {
            ...
            audio = new Audio(aud_path + row.precedingaudio_parta.slice(0, -10) + '_' + row.Speaker + suffix + row.precedingaudio_parta.slice(-10)); 
            console.log(audio);
            audio.play();
        }).call(),
        newTimer('wait2', 7000).start().wait(),
        
        newImage('topleft', img_path + row.image_TL).print(),
        newImage('topright', img_path + row.image_TR).print(),
        ...
        newFunction('aud3', () => {
            ...
            audio = new Audio(aud_path + row.precedingaudio_partb.slice(0, -10) + '_' + row.Speaker + suffix + row.precedingaudio_partb.slice(-10)); 
            console.log(audio);
            audio.play();
        }).call(),
        newTimer('wait2', 7000).start().wait(),
        
        newImage('bottomleft', img_path + row.image_BL).print(),
        newImage('bottomright', img_path + row.image_BR).print(),
        ...
        newFunction('aud4', () => {
            ...
            audio = new Audio(aud_path + row.precedingaudio_partc.slice(0, -10) + '_' + row.Speaker + suffix + row.precedingaudio_partc.slice(-10)); 
            console.log(audio);
            audio.play();
        }).call(),
        newTimer('wait2', 8500).start().wait(),
        ...
        newFunction('aud5', () => {
            ...
            audio = new Audio(aud_path + row.critical_audio.slice(0, -4) + suffix + row.critical_audio.slice(-4)); 
            console.log(audio);
            audio.play();
        }).call(),
        newTimer('wait2', 2500).start().wait(),

        newButton('space', 'spacebar').center().print(),
        newKey(' ').wait(),
        
        getMediaRecorder('recorder').stop()
    ).log('trial_type', row.Trial_type).log('trial_num', trial_num).log('condition', row.Condition).log('critical_audio', row.critical_audio).log('target_image_position', row.target_image_position).log('TargetVerbalResponse_Color', row.TargetVerbalResponse_Color), 
);
